    ...
    def verify_component(
        ...
        for part_num in all_part_numbers:
            ...
            if temp_official_data and temp_official_data.get('found', False):
                ...
                # Try variations if OCR confidence is low OR medium (< 85%)
                # This helps correct common OCR errors like S2BP -> 328P
                ocr_confidence = extracted_data.get('confidence', 100)
                should_try_variations = ocr_confidence < 0.85  # More lenient threshold
                
                if should_try_variations:
                    print(f"   ℹ️ OCR confidence {ocr_confidence:.0%} - trying limited variations...")
        ...
        # If we found a datasheet, the part number is already validated by the search
        extracted_part = extracted_data.get('part_number', '')
        official_part = official_data.get('part_marking', '')
        
        # If we have a matched part number from search, that's already validated
        if matched_part_number:
            part_check = {'passed': True, 'score': 100.0, 'reason': 'Part number validated via datasheet search'}
            verification_result['checks_passed'].append('Part Number Match')
        else:
            part_check = self._verify_part_number(extracted_part, official_part)
            if extracted_part and not part_check['passed']:
                verification_result['checks_failed'].append('Part Number Mismatch')
                verification_result['anomalies'].append(
                    f"Part number mismatch: extracted '{extracted_part}' vs official '{official_part}'"
                )
        ...
        if images:
            ...
            # No images provided - skip print quality check
            quality_check = {'passed': True, 'score': 0.0, 'reason': 'No image data for analysis'}
            verification_result['detailed_scores']['print_quality'] = quality_check
